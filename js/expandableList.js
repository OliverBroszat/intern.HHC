...
function getRenderedContentArrayForExlContainer(exlContainer, data) {
	...
	for (var index in data) {
		...
		dataset['content-id'] = i;
		dataset['content-full-id'] = exlContainer.id+'-'+i;
...
function getExlDataArrayForContainerWithData(exlContainer, data) {
	...
	var exlData = {
		...
		'exl-size': size,
		'exl-data': data,
        'exl-numberOfItems': renderedExlContent.length,
		'exl-contentTemplate': getContentTemplateForExlContainer(exlContainer),
...
function createNewDeleteButton(exlContainerID, itemID) {
	...
	newDeleteButton.setAttribute('onClick', 'deleteListItemForContainerWithID("'+exlContainerID+'", '+itemID+');');
...
function updateItemCounterForID(exlContainerID) {
    var itemCounterNode = document.getElementById('exl-itemCounter-'+exlContainerID);
 ...
 * @param exlContainerID The exl-container's ID
 */
function addNewListItemForContainerWithID(exlContainerID) {

	var container = document.getElementById(exlContainerID);
	var contentTemplate = document.getElementById('contentTemplate-'+exlContainerID).value;

	var exlListNode = document.getElementById('exl-list-'+exlContainerID);
	var currentMaxID = parseInt(document.getElementById('exl-itemCounter-'+exlContainerID).value);
 ...
 */
function deleteListItemForContainerWithID(exlContainerID, listItemID) {
 ...
 */
function setupExlContainerWithData(exlContainer, data) {
	...
	var exlWrapperTemplate = getTemplateByName('__exl-template');
 ...
 * @param data Datastructure as json
 */
function setupExlContainerWithID(exlContainerID, data) {
	var exlContainer = document.getElementById(exlContainerID);
	setupExlContainerWithData(exlContainer, data);
}
 ...
 * @param data Datastructure as json
 */
function setupAllExlContainersWithData(data) {
	var elements = document.getElementsByTagName('exl-container');

	for (var i=0; i<elements.length; i++) {
		var container = elements[i];
		setupExlContainerWithData(container, data);
	}
}
